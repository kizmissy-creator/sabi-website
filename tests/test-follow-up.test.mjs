...
import submit from '../tester-site/netlify/functions/test-follow-up-submit.mjs';
...
process.env.TESTER_ACCESS_SECRET = 'fictional-test-access';
process.env.TESTER_SUBMISSION_SECRET = 'fictional-test-signing';
process.env.TESTER_APPS_SCRIPT_ENDPOINT = 'https://receiver.invalid/exec';
process.env.TESTER_FOLLOW_UP_ENABLED = 'true';
const identity = `TEST-CAREER-PARTNER.test_unit.${Math.floor(Date.now()/1000)+60}`;
const cookie = Buffer.from(identity).toString('base64url') + '.' + createHmac('sha256', process.env.TESTER_ACCESS_SECRET).update(identity).digest('base64url');
...
const request = (body=input, headers={}) => new Request('https://client.invalid/api/test-follow-up-submit', { method: 'POST', headers: { origin: 'https://client.invalid', cookie: 'sabi_tester_access='+cookie, ...headers }, body: JSON.stringify(body) });
...
delete process.env.TESTER_FOLLOW_UP_ENABLED;
assert.equal((await submit(request())).status,503);
assert.equal(calls,0);
process.env.TESTER_FOLLOW_UP_ENABLED = 'true';
assert.equal((await submit(request())).status,200);
assert.equal(delivered.testOnly,true);
assert.equal(delivered.signature, createHmac('sha256',process.env.TESTER_SUBMISSION_SECRET).update('follow-up-v1\n'+delivered.payload).digest('base64url'));
delete process.env.BRONAGH_FOLLOW_UP_ENABLED;
assert.equal((await submit(request({...input,testMode:false}))).status,400);
...
const properties = { TESTER_SUBMISSION_SECRET:process.env.TESTER_SUBMISSION_SECRET, BRONAGH_SUBMISSION_SECRET:process.env.BRONAGH_SUBMISSION_SECRET, BRONAGH_UPLOAD_FOLDER_ID:'private-client-root',TESTER_UPLOAD_FOLDER_ID:'private-test-root' };
...
const attemptedClientPayload = JSON.stringify({action:'career_partner_follow_up',submission:production,expiresAt:Date.now()+60000});
assert.throws(()=>context.receiveCareerFollowUp_({testOnly:true,payload:attemptedClientPayload,signature:createHmac('sha256',properties.TESTER_SUBMISSION_SECRET).update('follow-up-v1\n'+attemptedClientPayload).digest('base64url')}), /Tester credentials/);
assert.equal(files.length,3,'even a correctly signed tester request cannot create a client record');
