...
export function PageHeader({
  ...
  logoSrc = '/follow-up/sabi-mark-complete.png',
  progressPercent,
}: PageHeaderProps) {
  return (
      ...
      <div className="mx-auto flex max-w-6xl flex-col gap-5 px-5 py-5 sm:flex-row sm:items-center sm:justify-between sm:px-8">
        ...
        <div className="w-full sm:w-56">
          ...
          <div
            ...
            aria-valuemin={progressPercent === undefined ? 1 : 0}
            aria-valuemax={progressPercent === undefined ? totalSteps : 100}
            aria-valuenow={progressPercent === undefined ? currentStep : progressPercent}
          >
            <span
              className="block h-full rounded-full bg-gold-400 transition-[width]"
              style={{ width: `${progressPercent ?? (currentStep / totalSteps) * 100}%` }}
            />
          </div>
        </div>
      </div>
