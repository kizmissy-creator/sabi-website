  ...
  return <div className="min-h-screen">
    ...
    <main className="mx-auto max-w-4xl px-5 py-10 sm:px-8">
      ...
      <form onSubmit={review} className="mt-9 space-y-6" noValidate>
        {errors.length > 0 && <section id="enquiry-errors" role="alert" tabIndex={-1} className="rounded-2xl border-2 border-red-700 bg-red-50 p-5 text-red-950 outline-none focus:ring-4 focus:ring-red-300"><h2 className="text-xl font-extrabold">Please complete the required parts</h2><ul className="mt-3 list-disc space-y-1 pl-5">{errors.map((error) => <li key={error}>{error}</li>)}</ul></section>}
