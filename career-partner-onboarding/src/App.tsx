...
function App() {
  ...
  const [roles, setRoles] = useState<RepeatItem[]>([{ title: '', organisation: '', startMonth: '', startYear: '', endMonth: '', endYear: '', workPattern: '', weeklyHours: '', responsibilities: '', achievements: '', leaving: '' }])
      ...
      <section id="form-section" className="mx-auto max-w-5xl scroll-mt-4 px-5 py-10 sm:px-8">
        ...
        <form className="space-y-5" onSubmit={(event) => event.preventDefault()}>
          ...
          {step === 3 && <>
            ...
            {(answers.workHistoryMethod === 'Upload a document, then add or correct missing details' || answers.workHistoryMethod === 'Enter the relevant history here') && <WorkHistory items={roles} onChange={(index, key, value) => updateRepeat(setRoles, index, key, value)} onAdd={() => setRoles((items) => [...items, { title: '', organisation: '', startMonth: '', startYear: '', endMonth: '', endYear: '', workPattern: '', weeklyHours: '', responsibilities: '', achievements: '', leaving: '' }])} onRemove={(index) => setRoles((items) => items.filter((_, itemIndex) => itemIndex !== index))} />}
          ...
          {step === 8 && <>
            {choices('accessibilityGate', 'Would you like to tell SABI about practical communication, accessibility or support needs?', singleOptions.accessibilityGate, false, 'This question is required, but sharing any health, disability or neurodivergence information is optional. You do not need a diagnosis.', false)}
...
function WorkHistory({ items, onChange, onAdd, onRemove }: { items: RepeatItem[], onChange: (index: number, key: string, value: string) => void, onAdd: () => void, onRemove: (index: number) => void }) {
  ...
  return <Field label="Your work and other experience" help="Add paid work, self-employment, volunteering, caring or another substantial responsibility that may be useful."><div className="space-y-6">{items.map((item, index) => <section key={index} className="rounded-2xl border-2 border-teal-200 bg-teal-50/60 p-5" aria-label={`Experience ${index + 1}`}><div className="mb-5 flex items-center justify-between gap-4"><h4 className="font-display text-xl font-semibold text-teal-900">About this experience</h4>{items.length > 1 && <button type="button" onClick={() => onRemove(index)} className="font-bold text-teal-800 underline">Remove</button>}</div><div className="grid gap-4 sm:grid-cols-2"><TextInput id={`experience-${index}-title`} label="Job title or brief description" value={item.title ?? ''} onChange={(event) => onChange(index, 'title', event.target.value)} /><TextInput id={`experience-${index}-organisation`} label="Organisation or setting, if relevant" value={item.organisation ?? ''} onChange={(event) => onChange(index, 'organisation', event.target.value)} /><SelectField id={`experience-${index}-start-month`} label="Start month" value={item.startMonth ?? ''} options={monthOptions} onChange={(value) => onChange(index, 'startMonth', value)} /><SelectField id={`experience-${index}-start-year`} label="Start year" value={item.startYear ?? ''} options={yearOptions} onChange={(value) => onChange(index, 'startYear', value)} /><SelectField id={`experience-${index}-end-month`} label="End month" value={item.endMonth ?? ''} options={monthOptions} onChange={(value) => onChange(index, 'endMonth', value)} /><SelectField id={`experience-${index}-end-year`} label="End year" value={item.endYear ?? ''} options={yearOptions} onChange={(value) => onChange(index, 'endYear', value)} /><SelectField id={`experience-${index}-pattern`} label="Type of work" value={item.workPattern ?? ''} options={workPatternOptions} onChange={(value) => onChange(index, 'workPattern', value)} /><SelectField id={`experience-${index}-hours`} label="Usual hours" value={item.weeklyHours ?? ''} options={hourOptions} onChange={(value) => onChange(index, 'weeklyHours', value)} /></div><div className="mt-4 grid gap-4"><label className="block font-bold text-teal-950">What did you do?<textarea value={item.responsibilities ?? ''} onChange={(event) => onChange(index, 'responsibilities', event.target.value)} className="field field-auto-grow mt-2 min-h-32 font-normal" /></label><label className="block font-bold text-teal-950">Achievements or things that went well<textarea value={item.achievements ?? ''} onChange={(event) => onChange(index, 'achievements', event.target.value)} className="field field-auto-grow mt-2 min-h-32 font-normal" /></label><label className="block font-bold text-teal-950">Reason for leaving, if useful<textarea value={item.leaving ?? ''} onChange={(event) => onChange(index, 'leaving', event.target.value)} className="field field-auto-grow mt-2 min-h-24 font-normal" /></label></div></section>)}<button type="button" onClick={onAdd} className="rounded-xl border-2 border-teal-700 px-4 py-2 font-bold text-teal-800">+ Add another experience</button></div></Field>
}
