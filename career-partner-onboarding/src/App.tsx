...
const yearOptions = Array.from({ length: 80 }, (_, index) => String(new Date().getFullYear() + 1 - index))
const workPatternOptions = ['Full time', 'Part time', 'Contract', 'Temporary', 'Casual or zero-hours', 'Self-employed', 'Volunteer', 'Caring or unpaid work', 'Other']
...
function App() {
  ...
  const [roles, setRoles] = useState<RepeatItem[]>([{ title: '', organisation: '', startMonth: '', startYear: '', endMonth: '', endYear: '', isCurrent: '', workPattern: '', weeklyHours: '', responsibilities: '', achievements: '', leaving: '' }])
  ...
  useEffect(() => {
    try {
      ...
      const restoredAnswers = { ...initialAnswers, ...data.answers }
      const workHistoryChoices: Record<string, string> = {
        'Upload a reasonably complete CV or work-history document': 'Upload my CV or another document',
        'Upload a document, then add or correct missing details': 'Upload a document, then add or correct details',
        'Enter the relevant history here': 'Add my experience here',
      }
      restoredAnswers.workHistoryMethod = workHistoryChoices[String(restoredAnswers.workHistoryMethod)] ?? restoredAnswers.workHistoryMethod
    ...
    <main>
      ...
      <section id="form-section" className="mx-auto max-w-5xl scroll-mt-4 px-5 py-10 sm:px-8">
        ...
        <form className="space-y-5" onSubmit={(event) => event.preventDefault()}>
          <p className="text-sm font-semibold text-slate-700"><span className="font-extrabold text-red-700" aria-hidden="true">*</span><span className="sr-only">Asterisk:</span> Required question</p>
          ...
          {step === 0 && <>
            ...
            {choices('ageBand', 'Which age group applies to you?', singleOptions.ageBand, false, 'Career Support is available from age 16 at launch. A full date of birth is not required.', false)}
          ...
          {step === 3 && <>
            {choices('workHistoryMethod', 'How would you like to tell us about your experience?', ['Upload my CV or another document', 'Upload a document, then add or correct details', 'Add my experience here', 'I am not sure or would prefer to discuss it'], false, 'Experience can include jobs, self-employment, volunteering, caring, education and projects.', false)}
            {(answers.workHistoryMethod === 'Upload my CV or another document' || answers.workHistoryMethod === 'Upload a document, then add or correct details') && <SimulatedUpload label="Your CV or experience document" files={simulatedFiles} onFiles={addSimulatedFiles} onRemove={(name) => setSimulatedFiles((items) => items.filter((item) => item !== name))} />}
            {(answers.workHistoryMethod === 'Upload a document, then add or correct details' || answers.workHistoryMethod === 'Add my experience here') && <WorkHistory items={roles} onChange={(index, key, value) => updateRepeat(setRoles, index, key, value)} onAdd={() => setRoles((items) => [...items, { title: '', organisation: '', startMonth: '', startYear: '', endMonth: '', endYear: '', isCurrent: '', workPattern: '', weeklyHours: '', responsibilities: '', achievements: '', leaving: '' }])} onRemove={(index) => setRoles((items) => items.filter((_, itemIndex) => itemIndex !== index))} />}
            {answers.workHistoryMethod && area('employmentGaps', 'Are there any gaps or changes you would like help explaining?', 'Not every gap needs to appear on a CV, and we will never invent dates or activities.')}
          ...
          {step === 8 && <>
            {choices('accessibilityGate', 'Would you like to tell us about anything that would make this service easier to use?', singleOptions.accessibilityGate, false, 'You can mention communication, accessibility or practical support. Sharing health, disability or neurodivergence information is optional, and you do not need a diagnosis.', false)}
            ...
            {choices('consultation', 'How would you prefer any useful consultation or follow-up discussion?', singleOptions.consultation, false, undefined, false)}
...
function WorkHistory({ items, onChange, onAdd, onRemove }: { items: RepeatItem[], onChange: (index: number, key: string, value: string) => void, onAdd: () => void, onRemove: (index: number) => void }) {
  ...
  return <section aria-labelledby="experience-heading">
    <div className="mb-4 px-1">
      <h3 id="experience-heading" className="font-display text-2xl font-semibold text-ink">Your experience</h3>
      <p className="mt-2 leading-6 text-slate-600">Each card is for one job or other experience. This can include self-employment, volunteering, caring, education or a project.</p>
    </div>
    <div className="space-y-6">
      {items.map((item, index) => <article key={index} className="rounded-3xl border-2 border-teal-400 bg-white p-5 shadow-card sm:p-7" aria-label={`Job or experience ${index + 1}`}>
        <div className="mb-5 flex items-center justify-between gap-4"><h4 className="font-display text-2xl font-semibold text-teal-900">Job or experience {index + 1}</h4>{items.length > 1 && <button type="button" onClick={() => onRemove(index)} className="font-bold text-teal-800 underline">Remove</button>}</div>
        <div className="grid gap-4 sm:grid-cols-2"><TextInput id={`experience-${index}-title`} label="What did you do?" value={item.title ?? ''} onChange={(event) => onChange(index, 'title', event.target.value)} /><TextInput id={`experience-${index}-organisation`} label="Where did you do this?" value={item.organisation ?? ''} onChange={(event) => onChange(index, 'organisation', event.target.value)} /><SelectField id={`experience-${index}-start-month`} label="Start month" value={item.startMonth ?? ''} options={monthOptions} onChange={(value) => onChange(index, 'startMonth', value)} /><SelectField id={`experience-${index}-start-year`} label="Start year" value={item.startYear ?? ''} options={yearOptions} onChange={(value) => onChange(index, 'startYear', value)} /><SelectField id={`experience-${index}-end-month`} label="End month" value={item.endMonth ?? ''} options={monthOptions} onChange={(value) => onChange(index, 'endMonth', value)} /><SelectField id={`experience-${index}-end-year`} label="End year" value={item.endYear ?? ''} options={yearOptions} onChange={(value) => onChange(index, 'endYear', value)} /><SelectField id={`experience-${index}-pattern`} label="What kind of experience was this?" value={item.workPattern ?? ''} options={workPatternOptions} onChange={(value) => onChange(index, 'workPattern', value)} /><SelectField id={`experience-${index}-hours`} label="About how many hours did you usually do?" value={item.weeklyHours ?? ''} options={hourOptions} onChange={(value) => onChange(index, 'weeklyHours', value)} /></div>
        <div className="mt-5 grid gap-4"><label className="block font-bold text-teal-950">What did this involve?<textarea value={item.responsibilities ?? ''} onChange={(event) => onChange(index, 'responsibilities', event.target.value)} className="field field-auto-grow mt-2 min-h-36 font-normal" /></label><label className="block font-bold text-teal-950">What went well, or what are you proud of?<textarea value={item.achievements ?? ''} onChange={(event) => onChange(index, 'achievements', event.target.value)} className="field field-auto-grow mt-2 min-h-36 font-normal" /></label><label className="block font-bold text-teal-950">Why did you leave? <span className="font-normal text-slate-600">(if relevant)</span><textarea value={item.leaving ?? ''} onChange={(event) => onChange(index, 'leaving', event.target.value)} className="field field-auto-grow mt-2 min-h-28 font-normal" /></label></div>
      </article>)}
      <button type="button" onClick={onAdd} className="rounded-xl border-2 border-teal-700 px-4 py-2 font-bold text-teal-800">+ Add another job or experience</button>
    </div>
  </section>
}
...
function Repeating({ title, items, fields, onChange, onAdd, onRemove }: { title: string, items: RepeatItem[], fields: string[][], onChange: (index: number, key: string, value: string) => void, onAdd: () => void, onRemove: (index: number) => void }) {
  const itemLabel = title.startsWith('Qualifications') ? 'About this qualification or training' : 'About this opportunity'
  const addLabel = title.startsWith('Qualifications') ? 'Add another qualification or training' : 'Add another opportunity'
  return <Field label={title}><div className="space-y-5">{items.map((item, index) => <section key={index} className="rounded-2xl border-2 border-teal-200 bg-teal-50/60 p-5" aria-label={`${itemLabel} ${index + 1}`}><div className="mb-4 flex justify-between gap-4"><h4 className="font-display text-xl font-semibold text-teal-900">{itemLabel}</h4>{items.length > 1 && <button type="button" onClick={() => onRemove(index)} className="font-bold text-teal-800 underline">Remove</button>}</div><div className="grid gap-4 sm:grid-cols-2">{fields.map(([key, label]) => ['appeal', 'concerns'].includes(key) ? <TextArea key={key} id={`${title}-${index}-${key}`} name={key} label={label} hideLabel={false} value={item[key] ?? ''} onChange={(event) => onChange(index, key, event.target.value)} /> : <TextInput key={key} id={`${title}-${index}-${key}`} name={key} label={label} hideLabel={false} value={item[key] ?? ''} onChange={(event) => onChange(index, key, event.target.value)} />)}</div></section>)}<button type="button" onClick={onAdd} className="rounded-xl border-2 border-teal-700 px-4 py-2 font-bold text-teal-800">+ {addLabel}</button></div></Field>
}

function ServiceQuestions({ serviceId, answers, setValue, area, choices }: { serviceId: ServiceId, answers: Answers, setValue: (key: string, value: string | string[] | boolean) => void, area: (key: string, label: string, help?: string, optional?: boolean) => ReactNode, choices: (key: string, label: string, options: string[], multiple?: boolean, help?: string, optional?: boolean) => ReactNode }) {
  ...
  if (serviceId === 'starter-cv') return <>{choices('starterEligible', 'Does your experience mainly come from education, projects, volunteering, caring, interests or a small number of roles?', ['Yes', 'No', 'Not sure'], false, undefined, false)}{area('starterEvidence', 'What education, projects, responsibilities, interests or achievements should the CV use?', undefined, false)}</>
  return <>{area('cpDirection', 'What broad direction should the Career Partner documents support?', 'A role group or sector is enough.', false)}{area('cpSecondTarget', 'What vacancy or industry should the additional CV version target?')}{area('cpCoverTarget', 'What role, vacancy or industry should the included cover letter address?')}{choices('applicationExists', 'Do you have a substantially completed application for SABI to review?', ['Yes', 'Not yet', 'I need most or all of it written'], false, undefined, false)}{serviceId === 'career-partner-plus' && <>{choices('interviewKnown', 'Do you have an interview booked or a particular role to prepare for?', ['Interview booked', 'Particular role', 'Not yet'], false, undefined, false)}{answers.interviewKnown !== 'Not yet' && area('interviewDetail', 'Add the role, employer, interview date and supplied information')}</>}</>
}
